...
const mockDocuments: Document[] = [
  {
    ...
    description:
      "Estudo detalhado sobre a qualidade da água em municípios do norte do estado do Rio de Janeiro, com foco em indicadores de potabilidade e contaminação.",
  ...
  },
  {
    ...
    description:
      "Pesquisa sobre implementação de tecnologias sustentáveis na agricultura familiar da região norte fluminense.",
  ...
  },
  {
    ...
    description:
      "Análise geológica detalhada da formação e estrutura da Bacia de Campos, com implications para exploração de recursos naturais.",
  ...
  },
  {
    ...
    description:
      "Estudo sobre os efeitos das mudanças climáticas na fauna e flora da região norte fluminense.",
  ...
  },
  {
    ...
    description:
      "Pesquisa sobre novas tecnologias e metodologias para extração e processamento de petróleo e gás natural.",
    authors: [
      "Dr. Eduardo Santos",
      "Dra. Patricia Mendes",
      "Dr. Gabriel Torres",
  ...
  },
  {
    ...
    description:
      "Estudo sobre síntese e caracterização de novos materiais com propriedades específicas para aplicações industriais.",
...
export async function fetchDocuments(
  ...
  if (researchArea) {
    filteredDocuments = filteredDocuments.filter(
      (doc) => doc.researchArea === researchArea,
    );
  }
...
export function getFilterStats() {
  ...
  mockDocuments.forEach((doc) => {
    documentTypeCounts[doc.documentType] =
      (documentTypeCounts[doc.documentType] || 0) + 1;
    researchAreaCounts[doc.researchArea] =
      (researchAreaCounts[doc.researchArea] || 0) + 1;
  });

  return {
    documentTypes: Object.entries(documentTypeCounts).map(([name, count]) => ({
      name,
      count,
    })),
    researchAreas: Object.entries(researchAreaCounts).map(([name, count]) => ({
      name,
      count,
    })),
...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  const documentsThisMonth = mockDocuments.filter(
    (doc) => new Date(doc.createdAt) > lastMonth,
  ).length;
